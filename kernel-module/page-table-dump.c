...
typedef int (*ptdump_fn_t)( struct task_struct *task, pgd_t *start, 
             unsigned ptstart, unsigned ptend, void *userbuf);
...
///< the memory regions covered by the page-table entries
#define BASE_PAGE_SIZE   (1UL << 12)
#define LARGE_PAGE_SIZE  (1UL << 21)
#define HUGE_PAGE_SIZE   (1UL << 30)
#define SUPER_PAGE_SIZE  (1UL << 39)


///< calculation of the virtual address
...
static long export_numa_map(void *user_buf, unsigned user_buf_bytes)
{
    ...
    while (nr_visited < nr_node_ids) {
        ...
        copy_to_user(curr + offsetof(struct numa_node_info, node_start_pfn), 
                     &base, sizeof(base));
        copy_to_user(curr + offsetof(struct numa_node_info, node_end_pfn), 
                     &limit, sizeof(limit));
...
static int ptdump_crc32(struct task_struct *task, pgd_t *start, unsigned ptstart, 
                        unsigned ptend, void *userbuf)
{
...
static int ptdump_dump(struct task_struct *task, pgd_t *start, unsigned ptstart, 
                       unsigned ptend, void *userbuf)
{
    unsigned l2, l3, l4;
    union ptable_entry *pml4;
    ...
    copy_to_user((void *)((char *)ptbuf), (void *)&task->numa_pages_migrated, 
                sizeof(task->numa_pages_migrated));
...
static int ptdump_range(struct task_struct *task, pgd_t *start, 
                        unsigned pml4_start, unsigned pml4_end,
                        void *user_buf)
{
